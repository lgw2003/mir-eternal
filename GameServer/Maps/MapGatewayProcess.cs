    ...
    public static class MapGatewayProcess
    {
        ...
        private static void LoadContentGuards()
        {
            ...
            foreach (守卫刷新 守卫刷新 in 守卫刷新.DataSheet)
            {
                ...
                if (!mapInstance.CopyMap && 地图守卫.DataSheet.TryGetValue(守卫刷新.守卫编号, out var 对应模板))
        ...
        private static void LoadContentChests()
        {
            ...
            foreach (var chest in 宝箱刷新.DataSheet)
            {
                ...
                if (!map.CopyMap && 宝箱数据.DataSheet.TryGetValue(chest.宝箱编号, out var chestTemplate))
        ...
        private static void LoadContentSpawns()
        {
            ...
            foreach (怪物刷新 spawn in 怪物刷新.DataSheet)
            {
