...
namespace GameServer.Maps
{
	...
	public sealed class TrapObject : MapObject
	{
		...
		public void 被动触发陷阱(MapObject 对象)
		{
			...
			if (this.PassiveTriggerSkill != null && !对象.Died && (对象.ObjectType & this.陷阱模板.被动限定类型) != (GameObjectType)0 && 对象.IsSpecificType(this.TrapSource, this.陷阱模板.被动指定类型) && (this.TrapSource.GetRelationship(对象) & this.陷阱模板.被动限定关系) != (GameObjectRelationship)0 && (!this.陷阱模板.禁止重复触发 || this.被动触发列表.Add(对象)))
			{
