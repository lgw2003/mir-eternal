            ...
            if (task != null)
            {
                    ...
                    if (游戏技能.DataSheet.TryGetValue(a_00.触发技能名字, out 游戏技能 skill))
                    {
                        ...
                        if (a_00.计算触发概率)
                        {
                            flag = !a_00.计算幸运概率
                                ...
                                : ComputingClass.CheckProbability(ComputingClass.计算幸运(CasterObject[GameObjectStats.幸运]));
                        }
                ...
                else if (task is A_01_触发对象Buff a_01)
                {
                    ...
                    if (a_01.角色自身添加)
                    {
                        ...
                        if (flag3 && a_01.验证目标Buff && Hits.Values.FirstOrDefault((HitDetail O) => (O.Feedback & SkillHitFeedback.Miss) == SkillHitFeedback.正常 && (O.Feedback & SkillHitFeedback.丢失) == SkillHitFeedback.正常 && O.Object.Buffs.TryGetValue(a_01.目标Buff编号, out BuffData BuffData2) && BuffData2.当前层数.V >= a_01.所需Buff层数) == null)
                            flag3 = false;

                        if (flag3 && a_01.验证目标类型 && Hits.Values.FirstOrDefault((HitDetail O) => (O.Feedback & SkillHitFeedback.Miss) == SkillHitFeedback.正常 && (O.Feedback & SkillHitFeedback.丢失) == SkillHitFeedback.正常 && O.Object.IsSpecificType(CasterObject, a_01.所需目标类型)) == null)
                            flag3 = false;
                ...
                else if (task is C_03_计算对象位移 c_03)
                {
                    ...
                    byte b2 = (byte)((((ItSelf位移次数 != null) ? ItSelf位移次数.Length : 0) > SkillLevel) ? c_03.自身位移次数[SkillLevel] : 0);
                    ...
                    else if (c_03.角色ItSelf位移)
                    {
                        ...
                        int num16 = ((((ItSelf位移距离 != null) ? ItSelf位移距离.Length : 0) > SkillLevel) ? c_03.自身位移距离[SkillLevel] : 0);
